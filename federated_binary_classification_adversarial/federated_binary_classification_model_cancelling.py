...
if __name__ == "__main__":
    ...
    csv_result_path = cwd + os.sep + "binary_class_cancelling_new.csv"
    if os.path.isfile(csv_result_path):
        ...
        for agg in AggregationMechanism:
            ...
            for i in range(0, max_adversaries + 1):
                cp_filename = f'{cwd}{os.sep}binary_class_cancelling_{agg.value}_{str(i)}.pt'
                train_devices = []
                for device in RaspberryPi:
    ...
    FederationUtils.visualize_adversaries_model_poisoning(df, title="Model Cancelling Attack on MLP",
                                                          save_dir='result_plot_binary_classification_model_cancelling.png')
